...
public void IncreaseCleanlinessAndHappiness(int cleanlinessAmount, int happinessAmount)
{
    ...
    private void InitializeIncidentCards()
    {
        incidentCards = new List<Card>()
        {
            new Card(
                "Yangın",
    ...
    {
        return new Effect(
            Effect.ConvertToRandomValue(), // Mutluluk
            Effect.ConvertToRandomValue(), // Temizlik
            Effect.ConvertToRandomValue(), // Güç
            Effect.ConvertToRandomValue()  // Para
        );
    }
}
...
public class Effect
{
    ...
    {
        return random.Next(5, 26); // 5 ile 25 arasında rastgele değer
    }
}